...
public class Trick {
  ...
  /**
   * To find out if all cards are none-trump in the trick
   *
  ...
  public Card winner() {
    try {
      ...
      /**
       * if the trick is a trump trick (all cards are trump) or none of the
       * cards are trump the winner is the higher value
       */
      if (card1.getSuit() == trumpSuit) {
  ...
  /**
   * Add the given card to the next empty position
   * @param card
   * @throws TrickIsFullException 
   */
  public void addCard(Card card) throws TrickIsFullException {
    if (card1 == null) {
      setCard1(card);
    } else if (card2 == null) {
      setCard2(card);
      ...
      throw new TrickIsFullException("The trick is already full. You can not add the card ".concat(String.valueOf(card)));
    }
  }
  ...
  /**
   * To check if all cards are set.
   * @return true if none if the cards is null.
   */
  public boolean isFull() {
    return card1 != null && card2 != null && card3 != null && card4 != null;
  }
